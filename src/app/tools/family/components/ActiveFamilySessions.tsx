...
export function ActiveFamilySessions() {
  ...
  return (
    <div className="space-y-2">
      <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide">На этом устройстве</p>
      {entries.map((entry) => (
        <Link
          key={entry.key}
          href={entry.href}
          className="block rounded-lg border border-emerald-200 bg-emerald-50/80 px-3 py-2 touch-manipulation active:bg-emerald-100/80 transition-colors"
        >
          <div className="flex items-start justify-between gap-2 min-w-0">
            <div className="min-w-0 flex-1">
              <p className="font-medium text-xs text-gray-900 truncate">{entry.roomName}</p>
              <p className="text-[11px] text-gray-600 mt-0.5 truncate">
                {entry.memberName}
                <span className="text-gray-400"> · </span>
                {entry.roleLabel}
              </p>
              <p className="text-[10px] text-emerald-700 mt-0.5">{entry.hint}</p>
            </div>
            <span className="shrink-0 text-emerald-600 text-[11px] font-medium pt-0.5">Войти →</span>
          </div>
        </Link>
      ))}
    </div>
