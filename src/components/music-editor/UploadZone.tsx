...
const AUDIO_ACCEPT =
  "audio/*,.mp3,.wav,.m4a,.aac,.ogg,.flac,.mp4,.caf,audio/mpeg,audio/wav,audio/x-m4a,audio/mp4";
...
function pickAudioFiles(fileList: FileList | File[] | null): File[] {
  if (!fileList || fileList.length === 0) return [];
  return Array.from(fileList).filter((file) => {
    const type = file.type.toLowerCase();
    if (type.startsWith("audio/")) return true;
    return /\.(mp3|wav|m4a|aac|ogg|flac|mp4|caf)$/i.test(file.name);
  });
}
...
export function UploadZone({
  ...
  const handleFiles = useCallback(
    (fileList: FileList | File[] | null) => {
      ...
      const audioFiles = pickAudioFiles(fileList);
  ...
  const handleInputChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // Сброс обязателен на Android — иначе повторный выбор файла не срабатывает
      e.target.value = "";
    },
    [handleFiles],
  );
    ...
    <div className="flex flex-col flex-1 overflow-y-auto">
      <div className="max-w-2xl mx-auto w-full px-4 py-4 space-y-3">
        ...
        >
          ...
          <label
            htmlFor={inputId}
            className={`inline-block px-4 py-1.5 rounded-lg text-xs font-semibold bg-gray-900 hover:bg-gray-700 text-white transition-colors ${
              disabled ? "opacity-50 pointer-events-none" : "cursor-pointer"
            }`}
          >
            Выбрать файлы
