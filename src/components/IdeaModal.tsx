...
export default function IdeaModal({ open, onClose }: Props) {
  ...
  const [form, setForm] = useState({ idea: "", name: "", contact: "", website: "" });
  ...
  useEffect(() => {
    if (open) {
      ...
      setLoading(false);
      setError(null);
      setForm({ idea: "", name: "", contact: "", website: "" });
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.idea.trim() || !form.name.trim() || loading) return;
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl border border-gray-200 shadow-2xl shadow-black/10 overflow-hidden">
        ...
        {sent ? (
          <div className="px-6 py-10 flex flex-col items-center text-center gap-3">
            ...
            <p className="font-semibold text-gray-900">Идея отправлена</p>
            <p className="text-sm text-gray-500 leading-relaxed">
              Спасибо! Мы получили ваше предложение и рассмотрим его.
            </p>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3 pt-1">
              <button
                type="submit"
                disabled={!form.idea.trim() || !form.name.trim() || loading}
            ...
            <p className="text-[10px] text-gray-400 text-center">
              Идея отправится команде QHub напрямую
            </p>
          </form>
        )}
      </div>
