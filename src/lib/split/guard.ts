...
export function jsonSplitError(err: unknown): Response {
  ...
  if (err instanceof Error) {
    const map: Record<string, [string, number]> = {
      ...
      operation_not_found: ["Операция не найдена", 404],
      not_settlement_recipient: ["Подтвердить получение может только получатель", 403],
      not_withdrawal_recipient: ["Подтвердить получение может только получатель", 403],
      asset_custodian_required: ["Списать может только держатель кассы или владелец комнаты", 403],
