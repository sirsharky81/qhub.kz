...
/** iOS 26+ Safari/PWA: setSinkId works only on a WebAudio relay element, not on raw WebRTC tracks. */
...
export async function findIosAudioOutputId(speaker: boolean): Promise<string | undefined> {
  ...
  const matchesSpeaker = (label: string) =>
    /speaker|громк|built.?in.?speaker|main/i.test(label);
  const matchesEarpiece = (label: string) =>
    /receiver|earpiece|трубк|iphone/i.test(label);
