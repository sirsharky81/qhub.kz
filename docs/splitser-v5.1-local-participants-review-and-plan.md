...
| C | Второй девайс | **Только если авторизован в whitelist — иначе нет.** Один Participant не открывается произвольно с любого устройства. Допуск второго (и далее) устройства = явная запись в whitelist этого Participant / комнаты. Без записи в whitelist — отказ. Механика whitelist (device id / session / linkedUserId) — в спеке Phase 1–2; не «шаринг одной ссылки на всех» |
| D | Locals при создании комнаты | **После**, уже в комнате («Добавить участника») |

### Уточнение C — whitelist второго устройства

Сценарий: тот же человек хочет открыть **тот же** Connected-seat на другом телефоне/планшете.

- **По умолчанию: нет** (один активный доступ без явного допуска).
- **Да — только после whitelist:** устройство / сессия / идентификатор явно добавлены в allow-list Participant (или комнаты).
- Произвольный повторный `join` по старой общей ссылке **не** должен выдавать тот же seat второй сессии.
- Детали хранения whitelist (поля, кто может добавлять записи — owner vs сам participant) — в Phase 1–2 store; не блокирует local participants MVP, но правило продукта зафиксировано.
...
- §4 + ответы A–D зафиксированы (2026-07-15).
- Перед кодом: спека claim (идемпотентность / погашение токена).
...
**Следующий шаг разработки:** Phase 1 — store/types (local participant + seat-bound invite + claim).
