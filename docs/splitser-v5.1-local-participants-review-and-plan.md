...
**Документ / план.** Код ещё не менялся.
...
## 4. Предлагаемые продуктовые решения (§0)

Зафиксировать при старте реализации (как в v5.0):

| # | Вопрос | Предложение |
| --- | --- | --- |
| 1 | Можно ли начать без invite других? | **Да** — add local → расходы сразу |
| 2 | Owner при создании | Создатель комнаты = `connected` + `role: owner`. Ownerless / fully-local room **нет** |
| 3 | Кто может добавлять/редактировать locals | Любой **connected** член комнаты (как любой может invite сегодня). Удаление — см. п.6 |
...
| 5 | Старый generic invite (`/invite` без seat) | Оставить как «добавить нового connected без заранее созданного local» **или** deprecate в пользу «сначала local, потом invite». Рекомендация: оба пути; UI по умолчанию — seat-bound |
| 6 | Удаление local с историей | **Нельзя hard-delete**, если есть ops/expenses/settlements. Только rename / soft-leave. Merge двух seats — post-MVP |
| 7 | `paidBy` / custodian = local | **Разрешено** (actor connected вводит факт «заплатил X») |
| 8 | Аватар | MVP: поле опционально, UI можно отложить; не блокер |
| 9 | Бренд в копирайте | «Пользователь QHub Split» / «Локальный участник» / «Приглашение отправлено» — не «Splitser» в UI |
| 10 | Reusable invite | Seat-bound invite: **одноразовый claim** (после connect токен гасится). Снижает двойные seats |

### Открытые вопросы (нужен короткий ок от продукта)

| # | Вопрос | Варианты |
| --- | --- | --- |
| A | Может ли local стать owner? | По умолчанию **нет** — owner всегда connected |
| B | Может ли connected «отвязаться» обратно в local? | Рекомендация MVP: **нет** (только leave). Detach — later |
| C | Второй девайс того же человека | Сейчас 1 token / member. Share token vs re-claim — отдельное решение; не смешивать с v5.1 MVP |
| D | Список участников на create room | Только owner; locals добавляются после в комнате (проще) |
