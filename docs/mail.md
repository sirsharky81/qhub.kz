...
**Outlook (Windows / iPhone):** на сервере только **`Sent Items`**. Дубликат **`Sent`** ломает iOS Outlook.

**iPhone — очистить «Исходящие» (обязательно):**
1. Включить **режим полёта**
2. Outlook → **Исходящие** → удалить все письма (**не** отправлять)
3. Выключить режим полёта
4. Потянуть список папок вниз (обновить) — лишняя **Sent** должна исчезнуть
