...
**Outlook (Windows / iPhone):** папка отправленных на сервере — **`Sent Items`** (не `Sent`). Если письма **застревают в «Исходящих»** и **дублируются** — Outlook повторно шлёт из очереди:

1. **Остановить дубли (iPhone):** Outlook → **Исходящие** → смахнуть письмо → **Удалить** (не «Отправить»). Либо удалить аккаунт из Outlook и добавить заново после шага 2.
2. **На сервере:** `bash scripts/mail/mail-init-folders.sh boris@qhub.kz` — создаст/переименует в `Sent Items`.
3. **Windows:** Другие настройки → Папки → Отправленные = **`Sent Items`**.
4. **iPhone:** после переименования на сервере — удалить аккаунт в Outlook → добавить IMAP снова (или использовать **Apple Mail**).

Dovecot помечает `Sent Items` / Drafts / Trash как special-use — Outlook подхватывает после переподключения.
