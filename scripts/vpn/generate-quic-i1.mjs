#!/usr/bin/env node
/** Generate AmneziaWG I1 QUIC mimicry (SagePtr mini_quic_generator, MIT). */
...
function quicTlsClientHelloSniOnly(sni, predefinedRandom = null) {
    ...
    if (!predefinedRandom) crypto.getRandomValues(randomBytes);
...
/** @param {string} sni @param {number} level */
export async function generateQuicI1(sni = "7-zip.org", level = 3) {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
    const sni = process.argv[2] || "7-zip.org";
    const level = parseInt(process.argv[3] || "3", 10);
    generateQuicI1(sni, level).then((i1) => {
